...
console.log('publish_module.cjs', { slug });

copy_dist_files(slug);

function copy_dist_files(slug) {
	const entry_point = get_entrypoint(slug);
	if (!entry_point) {
		return console.error('entry point not found', slug);
	}

	const dist_path = `dev/modules/${slug}/dist`;
